  ...
  $.ajax({
    ...
    success: function (data, status, xhr) {
      ...
      if (responseCode == "200") {
        ...
        if (myObj.events.length == 0) {
          result =
            '<div class="alert alert-danger d-flex align-items-center" role="alert"><svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Danger:"><use xlink:href="#exclamation-triangle-fill"/></svg><div>No events were found matching your criteria</div></div>';
        } else {
          if ($("input[name='viewRadio']:checked").val() == "table") {
            result =
              '<thead class="table-light"><tr><th>Route</th><th>Segment</th><th>Cause</th><th>Severity</th><th>Description</th><th>Created</th><th>Last Updated</th><th>More Details</th></tr></thead>';
          } else {
            result =
        ...
        for (x in myObj.events) {
          for (j in myObj.events[x].roads) {
            ...
            if (typeof myObj.events[x].roads[j].to === "undefined") {
              ...
              segment =
                myObj.events[x].roads[j].from +
                " to " +
                myObj.events[x].roads[j].to;
            }

            myObj.events[x].event_type = myObj.events[x].event_type.replace(
              "_",
              " ",
            );

            if (myObj.events[x].event_subtypes != null) {
              for (const i in myObj.events[x].event_subtypes) {
                myObj.events[x].event_subtypes[i] = myObj.events[
                  x
                ].event_subtypes[i].replace("_", " ");
                myObj.events[x].event_subtypes[i] =
                  " - " + myObj.events[x].event_subtypes[i];
              }
            }
            ...
            if ($("input[name='viewRadio']:checked").val() == "table") {
              ...
              if (myObj.events[x].severity == "MAJOR") {
                result +=
                  ...
                  (myObj.events[x].event_subtypes ?? "") +
                  "</td>" +
                  "<td>" +
                  myObj.events[x].severity +
                  "</td>" +
                  "<td>" +
                  ...
                  '</td><td><a href="https://www.drivebc.ca/~' +
              ...
              if (myObj.events[x].severity == "MAJOR") {
                result +=
                  '<div class="col"><div class="card text-bg-danger"><div class="card-body"><h5 class="card-title">' +
                  myObj.events[x].roads[j].name +
                  " " +
                  dirPhrase +
                  "</h5>" +
    ...
    error: function (xhr, status, error) {
      ...
      if (xhr.status == "400") {
        ...
        $("#submit").html("View Events");
        $("#submitError").html(
          '<div class="alert alert-danger d-flex align-items-center" role="alert"><svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Danger:"><use xlink:href="#exclamation-triangle-fill"/></svg><div><strong>(API Error 400 - Bad Request)</strong> Likely an invalid query. Please try again.</div></div>',
        );
        $("#submitError").show();
        ...
        $("#submit").html("View Events");
        $("#submitError").html(
          '<div class="alert alert-danger d-flex align-items-center" role="alert"><svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Danger:"><use xlink:href="#exclamation-triangle-fill"/></svg><div><strong>(API Error 401 - Forbidden)</strong> Invalid Credentials. Please try again.</div></div>',
        );
        $("#submitError").show();
        ...
        $("#submit").html("View Events");
        $("#submitError").html(
          '<div class="alert alert-danger d-flex align-items-center" role="alert"><svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Danger:"><use xlink:href="#exclamation-triangle-fill"/></svg><div><strong>(API Error 404 - Not Found)</strong> The Endpoint URL could not be found. Please try again.</div></div>',
        );
        $("#submitError").show();
      } else if (xhr.status == "429") {
        // If API responds with "too many requests" error
        $("#tableRadio").attr("disabled", false);
        $("#cardRadio").attr("disabled", false);
        $("#submit").attr("disabled", false);
        $("#submit").html("View Events");
        $("#submitError").html(
          '<div class="alert alert-danger d-flex align-items-center" role="alert"><svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Danger:"><use xlink:href="#exclamation-triangle-fill"/></svg><div><strong>(API Error 429 - Too many requests)</strong> Please wait 30 seconds and try again.</div></div>',
        );
        $("#submitError").show();
        ...
        $("#submit").html("View Events");
        $("#submitError").html(
          '<div class="alert alert-danger d-flex align-items-center" role="alert"><svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Danger:"><use xlink:href="#exclamation-triangle-fill"/></svg><div><strong>(Unhandled exception)</strong> Please try again.</div></div>',
        );
        $("#submitError").show();
      }
    },
    ...
    if ($("#highwaySelect").val() == "all") {
      if ($("input[name='severityRadio']:checked").val() == "all") {
        ...
        queryURL +=
          ...
          $("#areaSelect").val() +
          "&severity=" +
          $("input[name='severityRadio']:checked").val();
      }
    } else {
      if ($("input[name='severityRadio']:checked").val() == "all") {
        queryURL +=
